 ...
 * Clock speeds are determined by your parts - check their data sheets. Standard default is 100KHz
 * 
 ...
 * 
 * I2C is very useful when you have multiple slave devices. All devices will share the SDA and SCL lines.
 * Each slave has a unique identifier, which is 7 or 8 bits. If it's 7 bits, the LSB is a Read or Write bit. 
 * The Arduino library supports sending up to 32 bytes in one transaction. 
 ...
 * The lines are open drain, so they require pull up or pull down resistors. There are weak ones built into the ESP32, but you get a longer setup time when toggling, 
 * and therefore need a lower clock speed. Sometimes modules come with resistors built in. 
